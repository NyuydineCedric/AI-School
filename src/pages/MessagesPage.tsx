    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <main className="flex-1 p-6 flex gap-4 overflow-hidden">
        {/* Conversation list — full width on mobile until a chat is opened,
            a fixed 72 rail alongside the chat panel from md up. */}
        <div
          className={`${
            activeId ? "hidden md:flex" : "flex"
          } w-full md:w-72 bg-white border border-slate-200 rounded-xl flex-col overflow-hidden shrink-0`}
        >
          <div className="p-3 border-b border-slate-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-8 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
              />
            </div>
          </div>
              ...
              filteredConversations.map((c) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 text-left px-4 py-3 border-b border-slate-50 transition-colors ${
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-full ${avatarColor(
                        c.name,
                      )} text-white text-xs font-semibold flex items-center justify-center shrink-0`}
                    >
                      {initials(c.name)}
                    </div>
